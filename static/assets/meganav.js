/* Osmo Supply — Mega Navigation (Directional Hover). Same component as on evolako.rs.
   Requires GSAP (loaded from /assets/vendor/gsap.min.js). Data attributes are contract — do not rename. */
function initMegaNavDirectionalHover() {
  ...
  // Close the mobile menu when a link inside it is followed (static site: same-page anchors).
...
document.addEventListener('DOMContentLoaded', function() {
  if (!document.querySelector('[data-menu-wrap]')) return;
  if (typeof gsap === 'undefined') {
    // GSAP did not load: the animated panels stay hidden, so the toggles become plain links to their hubs.
    document.documentElement.classList.add('no-gsap');
    document.querySelectorAll('[data-dropdown-toggle][data-fallback-href]').forEach(function (b) {
      b.addEventListener('click', function () { window.location.href = b.getAttribute('data-fallback-href'); });
    });
    var burgerEl = document.querySelector('[data-burger-toggle]');
    var innerEl = document.querySelector('[data-nav-list]');
    if (burgerEl && innerEl) burgerEl.addEventListener('click', function () {
      var open = innerEl.style.visibility === 'visible';
      innerEl.style.visibility = open ? '' : 'visible';
      innerEl.style.opacity = open ? '' : '1';
      burgerEl.setAttribute('aria-expanded', open ? 'false' : 'true');
    });
    return;
  }
